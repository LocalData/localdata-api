  ...
  suite('POST', function () {
    ...
    test('Posting JSON to /surveys/' + surveyId + '/responses with an info field after not having one', function (done) {
      ...
      fixtures.clearResponses(surveyId, function() {
        ...
        request.post({
          ...
          var data = fixtures.makeResponses(1, {
            includeInfo: true,
            parcelId: body.responses[0].parcel_id
          });
    ...
    test('Posting a file without a human-readable name to /surveys/' + surveyId + '/responses', function (done) {
      ...
      fixtures.clearResponses(surveyId, function() {
        ...
        var iffy_response = fixtures.makeResponses(1);
        delete iffy_response.responses[0].geo_info.humanReadableName;
        ...
        var req = request.post({
          ...
          var i;
          for (i = 0; i < iffy_response.responses.length; i += 1) {
            // Source
            assert.deepEqual(iffy_response.responses[i].source, body.responses[i].source, 'Response differs from posted data');
            // Centroid
            assert.deepEqual(iffy_response.responses[i].geo_info.centroid,
                             body.responses[i].geo_info.centroid,
                             'Response centroid differs from posted data');
            // Geometry
            assert.deepEqual(iffy_response.responses[i].geo_info.geometry,
                             body.responses[i].geo_info.geometry,
                             'Response geometry differs from posted data');

            // Human-readable name
            should.not.exist(body.responses[i].geo_info.humanReadableName);

            // Object ID
            assert.deepEqual(iffy_response.responses[i].parcel_id, body.responses[i].parcel_id, 'Response differs from posted data');
            assert.deepEqual(iffy_response.responses[i].object_id, body.responses[i].object_id, 'Response differs from posted data');

            // Answers
            assert.deepEqual(iffy_response.responses[i].responses, body.responses[i].responses, 'Response differs from posted data');
        ...
        var dataAsString = JSON.stringify(iffy_response);
        form.append('data', dataAsString);
      });
    });
