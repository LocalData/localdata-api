...
suite('Responses', function () {
  suite('POST', function () {
    ...
    test('Posting JSON to /surveys/' + surveyId + '/responses', function (done) {
      request.post({url: url, json: data}, function (error, response, body) {
        ...
        var i;
        for (i = 0; i < data.responses.length; i += 1) {
          assert.deepEqual(data.responses[i].source, body.responses[i].source, 'Response differs from posted data');
          assert.deepEqual(data.responses[i].geo_info, body.responses[i].geo_info, 'Response differs from posted data');
          assert.deepEqual(data.responses[i].parcel_id, body.responses[i].parcel_id, 'Response differs from posted data');
          assert.deepEqual(data.responses[i].parcel_id, body.responses[i].parcel_id, 'Response differs from posted data');
          assert.deepEqual(data.responses[i].responses, body.responses[i].responses, 'Response differs from posted data');
  ...
  suite('DEL', function () {
    ...
    setup(function (done) {
      request.post({url: BASEURL + '/surveys/' + surveyId + '/responses', json: data}, function (error, response, body) {
        if (error) { done(error); }
  ...
  suite('GET', function () {
    ...
    suiteSetup(function (done) {
      request.post({url: BASEURL + '/surveys/' + surveyId + '/responses', json: data},
    ...
    test('Get all responses for a specific parcel', function (done) {
      request.get({url: BASEURL + '/surveys/' + surveyId + '/parcels/' + data.responses[1].parcel_id + '/responses'},
                  function (error, response, body) {
        should.not.exist(error);
        response.statusCode.should.equal(200);
        response.should.be.json;
        ...
        parsed.responses[0].parcel_id.should.equal(data.responses[1].parcel_id);
    ...
    test('Get a response', function (done) {
      request.get({url: BASEURL + '/surveys/' + surveyId + '/responses/' + id},
        ...
        should.deepEqual(parsed.response.source, data.responses[0].source);
        should.deepEqual(parsed.response.geo_info, data.responses[0].geo_info);
        should.deepEqual(parsed.response.responses, data.responses[0].responses);
        parsed.response.parcel_id.should.equal(data.responses[0].parcel_id);
    ...
    test('Get all responses in a bounding box', function (done) {
      var center = data.responses[0].geo_info.centroid;
