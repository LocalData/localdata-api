...
function simplifyLineString(coordinates) {
  ...
  for (i = 1; i < len; i += 1) {
    ...
    if (coordinates[i][0] !== last[0] &&
      ...
      console.log("Pushing coords at index", i, coordinates[i]);
      console.log("Last:                  ", i, last);

    }else {
      console.log("Not pushing coords at index", i, coordinates[i]);
      console.log("Last:                      ", i, last);
    }
  }
  return newCoords;
...
function createIndexedGeometry(geometry) {
  ...
  if (geom.type === 'LineString') {
    ...
    console.log("SWEEPLINE RESULTS", sweepline.isSimplePolygon(geom));
    ...
    console.log("Simplifying", geom.coordinates);
    geom.coordinates = [simplifyLineString(geom.coordinates[0])];
    console.log("Simplified to", geom.coordinates);
  }
  return geom;
}
...
responseSchema.pre('save', function addIndexedGeometry(next) {
  ...
  console.log("Indexed geom", this.indexedGeometry.coordinates);
