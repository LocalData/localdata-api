...
responseSchema.index({
  'properties.survey': 1,
  'properties.indexedGeometry': '2dsphere',
  'properties.entries.created': 1
});

// Index the survey ID + entry ID. We expose entry ID to clients.
responseSchema.index({
  'properties.survey': 1,
  'properties.entries._id': 1
});
...
// We use a sparse index because some entries might not correspond to a base
// feature. They will not have an object_id.
responseSchema.index({
  'properties.survey': 1,
...
responseSchema.methods.toUpsertDoc = function toUpsertDoc() {
  return this.toObject({
    transform: function transform(doc, ret, options) {
      delete ret._id;
      var entries = ret.properties.entries;
...
responseSchema.pre('save', function addIndexedGeometry(next) {
  ...
  this.properties.indexedGeometry = createIndexedGeometry(this.geometry);
