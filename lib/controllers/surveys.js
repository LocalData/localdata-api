...
exports.addUser = function addUser(req, res) {
  ...
  Survey.findOne({ id: surveyId }).select('users').exec(function(error, survey){
    User.findOne({ email: email }, function (error, user) {
      ...
      console.log("ADding user to survey: ERROR/USER", error, user);
      if (error) {
        res.send({ name: 'UserQueryError', message: 'We were not able to add the user at this time' }, 400);
        return;
      }
      if(!user) {
      ...
      Survey.update({ id: surveyId}, { users: survey.users}, function(err, doc) {
        if (err) {
          res.send({ name: 'SurveySaveError', message: "We couldn't add the user" }, 400);
          return;
        }
        res.send({ survey: doc });
        return;
