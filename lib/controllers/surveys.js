...
exports.addUser = function addUser(req, res) {
  ...
  function(error, results) {
    ...
    if (error) {
      res.status(500).send({ name: 'UserQueryError', message: 'We were not able to add the user at this time' });
      return;
    }
    if(!user) {
      res.status(400).send({ name: 'UserNotFoundError', message: 'User not found' });
      return;
    }

    // Add the user to the survey users
    if(! __.includes(survey.users, user._id)) {
    ...
    Survey.update({ id: surveyId}, { users: survey.users}, function(err, doc) {
      if (err) {
        res.status(500).send({ name: 'SurveySaveError', message: "We were not able to add the user at this time" });
        return;
      }
      res.send({ survey: doc });
      return;
...
exports.removeUser = function addUser(req, res) {
  ...
  function(error, results) {
    ...
    Survey.update({ id: surveyId }, { users: survey.users }, function(error, success) {
      if (error) {
        res.status(500).send({ name: 'SurveySaveError', message: "We were not able to remove the user at this time" });
        return;
      }
      res.sendStatus(204);
      return;
...
exports.post = function post(req, response) {
  ...
  Survey.create(data, function (error) {
    ...
    response.status(201).send({ surveys: output });
  });
};
