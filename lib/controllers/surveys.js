    ...
    if (survey === null) {
      ...
      Response.find({ survey: surveyId })
        ...
        for (i = 0; i < responses.length; i++) {
          ...
          if(__.has(processed, responses[i].object_id)) {
            continue;
          }
          processed[responses[i].object_id] = true;

          // Record the collector
          key = responses[i].source.collector;
          if(__.has(stats.collectors, key)){
            stats.collectors[key] += 1;
          }else {
            stats.collectors[key] = 1;
          }
