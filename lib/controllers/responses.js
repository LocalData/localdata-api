...
var uploader = multer({
  storage: multerS3({
    ...
    key: function(req, file, callback) {
      console.log('info at=responses event=file_received name=' + file.originalname);
      var split = file.originalname.split('.');
      var extension = split.slice(-1)[0];
      var unique = cuid.slug();
      var fileName = 'tmp/' + unique + '.' + extension;
...
function makeFilename(orig, surveyId, objectName, objectId) {
  ...
  console.log('info at=responses event=making_file_name name=' + orig);
...
exports.post = function post(req, res) {
  uploader(req, res, function(error) {
    ...
    // arrives as a string
    // TODO: may need to move this above the multer handler
    var files = req.files;
    if (files) {
      if (!req.body.data) {
    ...
    if (files) {
      async.each(files, function(file, callback) {
        ...
        // Save the file to S3
        client.copyFile(file.key, fileName, function (error, response) {
          if (error) {
            callback(error);
            return;
          }
