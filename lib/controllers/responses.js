...
var MAX_FILES = 20; // Max files allowed in a single response
...
exports.put = function patch(req, res) {
  var data = req.body;
  var responseId = req.params.responseId;
  ...
  set['entries.$.responses'] = data.responses;
...
function saveResponses(res, data, surveyId) {
  async.map(data, function (item, next) {
    ...
    res.status(201).send({ responses: docs });
  });
}
...
var uploader = multer({
  storage: multerS3({
    ...
    accessKeyId: settings.s3_key,
    dirname: settings.s3_dir,
    region: 'us-east-1',
    key: function(req, file, callback) {
      console.log("Starting to save", file.originalname, file);
      ...
      var fileName;
      try {
        fileName = makeFilename(file.originalname, surveyId, data[0].geo_info.humanReadableName, data[0].object_id);
        console.log("Made filename", fileName);
      } catch (e) {
        console.log("Error making filename", e);
        return callback(e);
      }
...
var storage = multer.diskStorage({
  destination: function (req, file, cb) {
    console.log("Test storage received this body:", req.body);
...
var testUpload = multer({ storage: storage }).any();

exports.post = function post(req, res) {
  var data = req.body.responses;
  var surveyId = req.params.surveyId;

  console.log("Post data in main handler: ", req.body);

  // Handle file uploads
  testUpload(req, res, function(error) {
    console.log("Done uploading file", error, req.files);
