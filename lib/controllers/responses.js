...
// Make it easier to use Document#toObject with map
...
function makeFeature(doc, n) {
  ...
  var entry = doc.properties.entries[n];
...
function responsesToGeoJSON(items, latestOnly) {
  ...
  for (i = 0; i < len; i += 1) {
    ...
    if (latestOnly) {
      ...
      numEntries = item.properties.entries.length;
...
exports.list = function list(req, res) {
  ...
  if (bbox !== undefined) {
    ...
    conditions['properties.indexedGeometry'] = {
      $geoIntersects: {
        $geometry: {
          type: 'Polygon',
  ...
  if (collector !== undefined) {
    conditions['properties.entries.source.collector'] = collector;
  }
  ...
  query.sort({ 'properties.entries.created': sort });
...
exports.get = function get(req, res) {
  Response.findOne({
    'properties.survey': req.params.surveyId,
    'properties.entries.id': req.params.responseId
  })
...
exports.del = function del(req, res) {
  ...
  .exec(function (error, doc) {
    ...
    if (doc.entries.length === 1) {
      ...
      Response.remove({ id: doc.id }, function (error) {
...
function saveResponses(res, data, surveyId) {
  ...
  async.map(data, function (item, next) {
    ...
    response.properties.entries.push(entry);
    ...
    if (object_id) {
      ...
      async.eachSeries(response._pres.save, function (f, next) {
        f.call(response, next);
      }, function (error){
        console.log(error);
      });

      // XXX We need a unique index on survey + object_id
      Response.update({
        survey: surveyId,
        object_id: object_id
      }, response.toUpsertDoc(), {
        upsert: true
      }, function (error) {
        if (error) {
          return next(error);
    ...
    res.send(201, { responses: docs.map(toObject) });
  });
