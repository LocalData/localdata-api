...
function responseToFeature(item) {
  var obj = {};
  obj.type = 'Feature';
  ...
  obj.properties = item;
  delete obj.properties.geo_info.geometry;
  return obj;
}
...
function resultsToGeoJSON(items) {
  return items.map(responseToFeature);
}
...
exports.list = function list(req, res) {
  ...
  if (paging === null && objectId === undefined && bbox === undefined && tile === undefined) {
    res.send(400, {
      name: 'QueryError',
  ...
  if (tile !== undefined) {
    conditions['geo_info.tiles.13'] = tile;
  }
  ...
  if (bbox === undefined && tile === undefined) {
  ...
  if (bbox !== undefined) {
    ...
    query.where('geo_info.centroid').within.box({ ll: [coords[0], coords[1]] , ur: [coords[2], coords[3]] });
  }

  //var output = [];
  //var stream = query.lean().stream();
  //stream.on('data', function (doc) {
  //  if (res.locals.format === 'geojson' || req.query.format === 'geojson') {
  //    output.push(responseToFeature(doc));
  //  } else {
  //    output.push(doc);
  //  }
  ...
  //}).on('close', function () {
  //  if (res.locals.format === 'geojson' || req.query.format === 'geojson') {
  //    res.send({type: 'FeatureCollection', features: output});
  //  } else {
  //    res.send({responses: output});
  //  }
  //});
  var ts = Date.now();
  ...
  .exec(function (error, responses) {
    ...
    console.log('Back from query: ' + (Date.now() - ts)); // XXX
    console.log('# of responses: ' + responses.length); // XXX
    if (res.locals.format === 'geojson' || req.query.format === 'geojson') {
      res.send({
        type: 'FeatureCollection',
        features: resultsToGeoJSON(responses)
      });
    } else {
