...
//    'sort',
//    'startIndex'
//    'responses[key]=val' -- accepts multiple
...
exports.list = function list(req, res) {
  ...
  if(filterParameters !== undefined) {
    console.log("GOt filter parameters", filterParameters);
    _.each(filterParameters, function(value, filter) {
      var filterPath;

      // Handle searching for responses where a field doesn't exist
      if(value === 'undefined') {
        // https://localhost:3443/api/surveys/286d3ad0-b4df-11e2-9f02-e3813ff8c9b3/responses?startIndex=0&count=100&responses[reviewed]=undefined
        filterPath = 'entries.responses.' + filter;
        // {'foo': { $elemMatch: { c: {$exists: false}}}}
        conditions['entries'] = { $elemMatch: {'responses.reviewed': { $exists: false } } };

        //conditions[filterPath] = {
        //  '$exists': false
        //};
...
exports.patch = function del(req, res) {
  var i;
  var data = req.body;
  ...
  Response.update({
    ...
    console.log("UPDATED ---", error, numCompleted);
    if (util.handleError(error)) { return; }
    res.send(200);
  });
};
...
exports.sendCSV = function sendCSV(req, res) {
  ...
  Survey.findOne({ id: surveyId }, 'slug exports').exec(function (error, doc) {
    ...
    function generate(removeStale) {
      ...
      update.$set['exports.' + type + '.name'] = name;
...
exports.handleShapefile = function handleShapefile(req, res) {
  ...
  Survey.findOne({ id: surveyId }, 'slug exports').exec(function (error, doc) {
    ...
    if (doc.exports && doc.exports.shapefile && doc.exports.shapefile.requested > (new Date(Date.now() - exportDuration))) {
