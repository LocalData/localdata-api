...
function exportResults(surveyId, filters, writer) {
  getAllResponses(surveyId, function (error, items) {
    ...
    for (i = 0; i < items.length; i += 1) {
      ...
      var row = [
        items[i].properties.object_id,
        items[i].properties.humanReadableName || '',
        items[i].properties.source.collector,
        items[i].properties.created.toISOString(), // Convert the date to ISO8601 format
        items[i].properties.source.type,
...
function saveResponses(res, data, surveyId) {
  ...
  async.map(data, function (item, next) {
    ...
    if(surveyId === '788d7d80-293d-11e4-abad-fd424515b10c' &&
      object_id === '007100030223') {
    ...
    if(surveyId === '30d50250-269b-11e4-b45c-5d65d83b39b6' && // cinci
      object_id === '008500030008') {
    ...
    // Something else
    if(surveyId === '30d50250-269b-11e4-b45c-5d65d83b39b6' && // cinci
      object_id === '008500030009') {

      console.log("FIXING A GEOMETRY", item.geo_info.geometry);
      item.geo_info.geometry.coordinates = [[
          [[-84.4924569968983,39.1099354195851],[-84.4924783649871,39.1099088576947],[-84.4925087621827,39.1098710720202],[-84.4927986261272,39.1098910894817],[-84.4927923926869,39.1099588599308],[-84.4924569968983,39.1099354195851]]
        ]];
    }
    ...
    if(surveyId === '30d50250-269b-11e4-b45c-5d65d83b39b6' && // cinci
      ...
      console.log("FIXING A GEOMETRY", item.geo_info.geometry);
      item.geo_info.geometry.coordinates = [[
        [[-84.4924069361472,39.1099976481362],[-84.49241720752,39.109984880715],[-84.4924569968983,39.1099354195851],[-84.4927923926869,39.1099588599308],[-84.4927864146469,39.1100238502712],[-84.4924069361472,39.1099976481362]]
      ]];
    }
    ...
    if (error) {
      if (error.name === 'ValidationError') {
        ...
        //console.log("Object ID", data[0].object_id);
        _.each(data, function(d){
          console.log("Object error", d.object_id);
        });
        console.log(error);
        res.send(500);
      }
...
exports.sendCSV = function sendCSV(req, res) {
  ...
  if (latest) {
    filters.push(exports.getLatestEntries);
  } else {
    filters.push(exports.getAllEntries);
  }
...
exports.sendKML = function sendKML(req, res) {
  var surveyId = req.params.surveyId;
  exportResults(surveyId, [exports.getAllEntries], makeKMLWriter(res));
};
