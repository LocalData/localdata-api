...
exports.del = function del(req, res) {
  ...
  .exec(function (error, doc) {
    ...
    if (doc.entries.length === 1) {
      // If there's just one entry, convert the whole parent object to a deleted
      // document.
      doc.properties.survey = {
        deleted: true,
      ...
      // Remove only the relevant entry data and place it in a deleted document.
      var entry = doc.entries.get({ _id: req.params.responseId });
      ...
      var deleted = new Response({
        ...
        geometry: doc.geometry,
        entries: [entry],
        indexedGeometry: doc.indexedGeometry,
      });

      // There might be a deleted document for this object_id already, so we use
      // an upsert.

      // Execute the pre-save middleware, which does not happen for
      // Model.update calls.
      this.applyPreSave(function (error) {
        if (error) {
          logfmt.error(error);
          if (util.handleError(error, res)) { return; }
        }
        ...
        async.parallel([
          doc.save.bind(doc),
          function (next) {
            Response.update(query, deleted.toUpsertDoc(), {
              upsert: true
            }, next);
          }
        ], function (error) {
