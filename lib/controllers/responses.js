...
exports.post = function post(req, res) {
  ...
  // Check if the file has been sent as a data URI.
  if (!files) {
    // We currently only support one response entry when a file is being
    // attached.
    if (data !== undefined && data.length > 0
        && data[0].files !== undefined && data[0].files.length > 0) {
      console.log('We are processing a base64-encoded file upload!');
      // TODO: support multiple files.
      var dataURI = data[0].files[0].data;
      var name = data[0].files[0].name;
      var pieces = dataURI.split(',');
      // Get the base64-encoded data.
      var fileDataString = pieces[1];
      // Get the content type.
      var type = pieces[0].split(';')[0].split(':')[1];
      console.log('Base64-encoded data: ');//XXX
      console.log(fileDataString); // XXX
      files = [{
        format: 'base64',
        data: new Buffer(fileDataString, 'base64'),
        name: name,
        type: type
      }];

      // Remove the files from the response entry.
      delete data[0].files;
    }
  }
  ...
  if (files) {
    ...
    async.each(fileList, function(file, callback) {
      ...
      makeFilename(file.name, surveyId, data[0].geo_info.humanReadableName, function (error, fileName) {
        ...
        // Respect the content-type set by the client.
        var headers = {};
        if (file.type) {
        ...
        if (file.format === 'base64') {
          // This is an in-memory, base64-encoded file.
          console.log('We are sending a Buffer to S3');
          client.putBuffer(file.data, fileName, headers, putDone);
        } else {
          // The file is on disk.
          client.putFile(file.path, fileName, headers, putDone);
        }
      });
...
exports.configureUploads = function configureUploads(settings) {
