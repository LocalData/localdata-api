...
function formatForAPI(doc, id) {
  // TODO: refactor so we can get the latest entry from a lean object and not
  // just a mongoose doc.
  var data;
  if (id && mongoose.Types.ObjectId.prototype.isPrototypeOf(id)) {
    data = doc.getSingleEntry(id);
  } else {
    data = doc.getLatestEntry();
  }
...
exports.list = function list(req, res) {
  ...
  .exec(function (error, responses) {
    ...
    if (res.locals.format === 'geojson') {
      ...
      res.send({ responses: responses.map(formatForAPI) });
    }
  });
};
...
exports.get = function get(req, res) {
  ...
  .exec(function (error, doc) {
    ...
    if (doc === null) {
      ...
      var response = formatForAPI(doc, new mongoose.Types.ObjectId(req.params.responseId));
...
function saveResponses(res, data, surveyId) {
  ...
  async.map(data, function (item, next) {
    ...
    function finish(error, doc) {
      ...
      next(null, formatForAPI(doc, entryId));
    }
