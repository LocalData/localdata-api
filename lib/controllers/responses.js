 ...
 * @return {Boolean}     Returns true if the name already exists
 */
 ...
 */
var makeFilename = function(file, response) {
  if (!file || !response) {
    return "ERROR";
  }
  ...
  checkIfNameExists(function(exists) {
...
exports.post = function post(req, res) {
  ...
  if(files) {
    ...
    for (var key in files) {
      ...
      // Get the name of the first object in the responses.
      ...
      // Save the file to S3
      client.putFile(file.path, file.name, function(error, response) {
        if(error) {
