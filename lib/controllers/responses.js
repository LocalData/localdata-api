 ...
 */
function makeFilename(file, response, done, suffix) {
  if (!file || !response) {
    return done({
      name: 'FileNamingError',
      message: 'File and response data required when generating a filename'
    });
  }
...
exports.post = function post(req, res) {
  ...
  if (files) {
    ...
    async.each(fileList, function(file, callback) {
      ...
      makeFilename(file, data[0], function (error, fileName) {
        if (error) {
      ...
      console.log('Saved files: ' + JSON.stringify(savedFilePaths));
