...
function getParcels(req, response, outputBuilder) {
  ...
  if (bbox !== undefined) {
    ...
    // Query for a parcel by ID
    queryConfig = {
      text: 'SELECT object_id, name1, name2, source, created, ST_AsGeoJSON(wkb_geometry) AS polygon, ST_AsGeoJSON(ST_Centroid(wkb_geometry)) AS centroid, GeometryType(wkb_geometry) as type FROM objects WHERE object_id = $1',
      values: [id],
      name: 'parcelBBoxQuery'
    };
  }else {
