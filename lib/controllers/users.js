...
passport.serializeUser(function (user, done) {
  console.log('info at=users event=serializing');
  return done(null, user);
});

passport.deserializeUser(function (user, done) {
  console.log('info at=users event=deserializing');
  return done(null, user);
});
...
passport.use(new LocalStrategy({
  ...
  function (username, password, done) {
    ...
    User.findOne({ email: username }, function (error, user) {
      ...
      if (!user) {
        console.log('info at=users event=bad_login issue=invalid_username username=' + username);
      ...
      if (!user.validPassword(password)) {
        console.log('info at=users event=bad_login issue=invalid_password username=' + username);
        return done(null, false, {
          name: 'BadRequestError',
...
exports.login = function login(req, res, next) {
  passport.authenticate('local', function(error, user, info) {
    ...
    if (info) {
      // TODO: coordinate a change with the dashboard, so that we can send JSON
      // over the wire.
  ...
  user.save(function (error, doc) {
    ...
    req.logIn(doc.toObject(), function (error) {
      if (error) {
        ...
        console.log('error at=users_create issue=unknown type=' + error.name + ' message="' + error.message + '"');
        res.sendStatus(401);
        return;
      }
...
exports.forgotPassword = function forgotPassword(req, response, next) {
  ...
  if (email === undefined) {
    response.status(400).send({ name: 'PasswordResetError', message: 'Email required' });
    return;
  }

  // Emails in the DB are lowercase
  ...
  User.findOne({ email: email }, function (error, user) {
    ...
    if(!user) {
      response.status(400).send({ name: 'PasswordResetError', message: 'Account not found' });
      return;
    }
    ...
    user.save(function (error, doc) {
      if (error) {
        console.log('error at=users_forgot_password issue=save_failure type=' + error.name + ' message="' + error.message + '"');
        return next(error);
      }
...
exports.resetPassword = function resetPassword(req, response, next) {
  ...
  if (!token) {
    response.status(400).send({ name: 'PasswordResetError', message: 'Password reset code required' });
    return;
  }
  if (!password) {
    response.status(400).send({ name: 'PasswordResetError', message: 'Password required' });
    return;
  }
  ...
  var user = User.findOne({email: email}, function(error, user) {
    ...
    if (user.reset === undefined) {
      response.status(400).send({ name: 'PasswordResetError', message: 'Reset token required' });
      return;
    }
    ...
    user.save(function (error) {
      ...
      req.logIn(user, function(error) {
        if (error) {
          console.log('error at=users_reset_password issue=unknown type=' + error.name + ' message="' + error.message + '"');
...
// Ensure that a user can use a survey
exports.ensureSurveyAccess = function ensurePermissions(req, res, next) {
