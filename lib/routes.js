...
function enforceHTTPS(req, res, next) {
  ...
  console.log(req.protocol, req.get('X-Forwarded-Protocol'), req.get('host'));
  if (!req.secure) {
...
exports.setup = function setup(app) {
  ...
  app.delete('/api/surveys/:surveyId', session, users.ensureAuthenticated, users.ensureSurveyAccess, surveys.del);
  ...
  app.delete('/api/surveys/:surveyId/users/:email', session, users.ensureAuthenticated, users.ensureSurveyAccess, surveys.removeUser);
  ...
  app.put('/api/surveys/:surveyId/responses/:responseId', session, users.ensureAuthenticated, users.ensureSurveyAccess, responses.put);
  app.post('/api/surveys/:surveyId/responses', responses.post);
  app.delete('/api/surveys/:surveyId/responses/:responseId', session, users.ensureAuthenticated, users.ensureSurveyAccess, responses.del);
  app.post('/api/surveys/:surveyId/responses', responses.post);
  ...
  app.post('/api/user', function(req, res, next) {
    console.log("Starting protocol", req.protocol);
    next();
  }, session, enforceHTTPS, function(req, res, next) {
    console.log("Got past enforceHTTPS");
    next();
  }, users.post);
